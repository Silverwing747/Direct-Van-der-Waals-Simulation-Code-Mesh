...
int main(int argc, char *argv[]) {
  ...
  // Initial Condition
  user.R1 = -INFINITY; user.C1[0] = 0.5; user.C1[1] = 0.5; user.C1[2] = 0.25; user.Aspect1 = 1.0;
  ...
  ierr = PetscOptionsReal("-Aspect1", "Model", __FILE__, user.Aspect1, &user.Aspect1, NULL);CHKERRQ(ierr);
  ...
  if(user.Case == 1){
    user.DomainSize[1][0] = 0; user.DomainSize[1][1] = 0.005 * user.F; user.MeshSize[1] = user.N;//user.N * (user.DomainSize[0][1] - user.DomainSize[0][0]);// X-axis
    user.DomainSize[0][0] = -0.005 * user.F; user.DomainSize[0][1] = 0.005 * user.F; user.MeshSize[0] = user.N*2;//user.N * (user.DomainSize[1][1] - user.DomainSize[1][0]);// Y-axis
    user.DomainSize[2][0] = 0.0; user.DomainSize[2][1] = 1.0; user.MeshSize[2] = user.N * (user.DomainSize[2][1] - user.DomainSize[2][0]);// Z-axis
  }else if(user.Case == 2){
    user.DomainSize[0][0] = 1e-2; user.DomainSize[0][1] = 2.0; user.MeshSize[0] = user.N * (user.DomainSize[0][1] - user.DomainSize[0][0]);// X-axis
    user.DomainSize[1][0] = 0.0; user.DomainSize[1][1] = 20.0; user.MeshSize[1] = user.N * (user.DomainSize[1][1] - user.DomainSize[1][0]);// Y-axis
    user.DomainSize[2][0] = 0.0; user.DomainSize[2][1] = 5.0; user.MeshSize[2] = user.N * (user.DomainSize[2][1] - user.DomainSize[2][0]);// Z-axis
  }else{
  ...
  PetscReal Refine_beyond_bubble = 0.6;

  PetscReal Scale = 8, ExtraFineScale = 1.0;
  PetscReal dx_fine = 1.0 / (double)user.N, dx_extraFine = dx_fine / ExtraFineScale, dx_coarse = dx_fine * Scale;

  PetscReal Lx_ExtraFine = 0.0;
  PetscReal Lx_fine = user.R1 + Refine_beyond_bubble;

  PetscInt N_ExtraFine_x = Lx_ExtraFine / dx_extraFine + 1;
  PetscInt N_fine_x = (Lx_fine - Lx_ExtraFine) / dx_fine + 1;
  PetscInt N_coarse_x = (user.DomainSize[0][1] - Lx_fine) / dx_coarse + 1;
  PetscInt N_all_x = N_coarse_x + N_fine_x + N_ExtraFine_x;
  PetscReal knots_x[N_all_x]; knots_x[0] = user.DomainSize[0][0];
  if (user.Case == 2){
    for (i = 0; i < N_ExtraFine_x; i++) knots_x[i+1] = knots_x[i] + dx_extraFine;
    for (N_ExtraFine_x; i < N_ExtraFine_x + N_fine_x; i++) knots_x[i+1] = knots_x[i] + dx_fine;
    for (i = N_ExtraFine_x + N_fine_x; i < N_all_x - 1; i++) knots_x[i+1] = knots_x[i] + dx_coarse;
  }

  PetscReal Lz_fine_start = user.C1[1] - user.R1 * user.Aspect1 - Refine_beyond_bubble;
  PetscReal Lz_fine_end = user.C1[1] + user.R1 * user.Aspect1 + Refine_beyond_bubble;
  if (Lz_fine_start < 0) Lz_fine_start = 0;
  PetscReal Lz_fine = Lz_fine_end - Lz_fine_start;

  PetscInt N_fine_z = Lz_fine / dx_fine + 1;
  PetscInt N_coarse_z = (user.DomainSize[1][1] - Lz_fine) / dx_coarse + 1;
  PetscInt N_all_z = N_fine_z + N_coarse_z;
  PetscReal knots_z[N_all_z]; knots_z[0] = user.DomainSize[1][0];
  for (i = 1; i < N_all_z; i++) knots_z[i] = 0;


  //PetscPrintf(PETSC_COMM_WORLD,"Lz_fine_start = %.6e; Lz_fine_end = %.6e; N_fine_z = %d; N_coarse_z = %d; N_all_z = %d;\n",Lz_fine_start,Lz_fine_end,N_fine_z,N_coarse_z,N_all_z);
  if (user.Case == 2){
    for (i = 0; i < N_all_z - 1; i++){
      if ((knots_z[i] > Lz_fine_start) && (knots_z[i] < Lz_fine_end))
        knots_z[i+1] += knots_z[i] + dx_fine;
      else
        knots_z[i+1] += knots_z[i] + dx_coarse;
    }
  }

/*   PetscInt N_BL_z = 0;
  PetscReal x_BL_min = 1.0;
  PetscReal x_BL_growth = 1.05;
  while (x_BL_min * PetscPowReal(x_BL_growth,N_BL_z) < dx_extraFine){
    N_BL_z++;
  }

  PetscPrintf(PETSC_COMM_WORLD,"N_BL_z = %d\n",N_BL_z);

  PetscReal Lz_ExtraFine = 0;
  PetscReal Lz_fine = user.C1[1] + user.R1 * user.Aspect1 + 0.5;

  PetscInt N_ExtraFine_z = Lz_ExtraFine / dx_extraFine + 1;
  PetscInt N_fine_z = (Lz_fine - Lz_ExtraFine) / dx_fine + 1;
  PetscInt N_coarse_z = (user.DomainSize[1][1] - Lz_fine) / dx_coarse + 1;
  PetscInt N_all_z = N_coarse_z + N_fine_z + N_ExtraFine_z + N_BL_z;
  PetscReal knots_z[N_all_z]; knots_z[0] = user.DomainSize[1][0];

  if (user.Case == 2){
    for (i = 0; i < N_BL_z; i++) knots_z[i+1] = knots_z[i] + PetscMin(x_BL_min * PetscPowReal(x_BL_growth,i),dx_extraFine);
    for (i = N_BL_z; i < N_ExtraFine_z + N_BL_z; i++) knots_z[i+1] = knots_z[i] + dx_extraFine;
    for (N_ExtraFine_z + N_BL_z; i < N_ExtraFine_z + N_BL_z + N_fine_z; i++) knots_z[i+1] = knots_z[i] + dx_fine;
    for (i = N_BL_z + N_ExtraFine_z + N_fine_z; i < N_all_z - 1; i++) knots_z[i+1] = knots_z[i] + dx_coarse;
    } */
  ...
  for(i=0;i<user.dim;i++){
    ...
    switch(user.BCType){
      ...
      case 9:
        ...
        break; 
      case 10:
        PetscPrintf(PETSC_COMM_WORLD,"Left pressure outlet, right free slip wall, top and bottom periodic/symmetry\n");
        if (i == 0){
          user.BCType_Identify[i][1] = 2; ierr = IGASetBoundaryValue(iga,i,1,user.dof-1,0.0); 
          user.BCType_Identify[i][0] = 2; ierr = IGASetBoundaryValue(iga,i,0,user.dof-1,0.0); 
        }else{
          ierr = IGASetBoundaryValue(iga,i,0,i+1,0.0); 
          ierr = IGASetBoundaryValue(iga,i,1,i+1,0.0);
    ...
    if (user.Case == 2){
      if (i == 0) ierr = IGAAxisInitBreaks(axis[i],N_all_x,knots_x,user.C);CHKERRQ(ierr);
      if (i == 1) ierr = IGAAxisInitBreaks(axis[i],N_all_z,knots_z,user.C);CHKERRQ(ierr);
    }
    else{
