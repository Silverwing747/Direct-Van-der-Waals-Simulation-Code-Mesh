...
struct _p_TS {
  ...
  /* ---------------------- Tianyi Added Parameters ---------------------------------*/
  PetscBool      recover; // Control read initial time derivative or not. 0 -> Not Read; 1 -> Recovery time derivative

  PetscReal		  time_next_print; // Desired print out time.
  PetscReal		  TianyiAdjustFactor1; // First time adjustment for adaptive time step, smaller than 1
  PetscReal	    TianyiAdjustFactor2; // Second time adjustment for adaptive time step, larger than 0

  PetscInt		  min_nits; // minimum wanted snes iteration number
  PetscInt		  min_lits; // minimum wanted ksp iteration number
  PetscInt		  max_nits; // maximum wanted snes iteration number
  PetscInt		  max_lits; // maximum wanted ksp iteration number

  PetscBool		  TianyiAdaptTime1; // Flag for first time adjustment or not;
  PetscBool		  TianyiAdaptTime2; // Flag for second time adjustment or not;
