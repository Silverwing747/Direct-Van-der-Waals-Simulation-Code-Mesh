...
static PetscErrorCode TSStep_Alpha(TS ts)
{
  ...
  PetscReal      next_time_step_dummy = ts->time_step;

  PetscReal		 maxtimestep = ts->time_next_print - ts->ptime; // Tianyi Modified, calculate the max time step allowed
  PetscInt		 lits, nits; // Tianyi Modified, Linear and non linear iteration numbers
  PetscBool		 PrintFileTimeAdjust = PETSC_FALSE; // Tianyi Modified, flag whether time is adjusted to print file
  ...
  // Tianyi Modified: Check if next time step will overshoot max time step allowed
  if (ts->time_step > maxtimestep) {
	  ts->time_step = maxtimestep;
	  PrintFileTimeAdjust = PETSC_TRUE;
    PetscPrintf(PETSC_COMM_WORLD, "Tianyi Modified: Time step adjust to print file. Real time step is %.3e\n",ts->time_step);
  }

  // Prepare to initialize the solver
  if (!ts->steprollback) {
  ...
  while (!ts->reason && th->status != TS_STEP_COMPLETE) {
    ...
    // Tianyi Modified: Allow the code to read initial time derivative
    if (ts->steprestart) {
      ierr = VecSet(th->V0,0.0);CHKERRQ(ierr);
      if(ts->recover && ts->vec_der) {
		    PetscPrintf(PETSC_COMM_WORLD, "Tianyi Modified: Initial derivative solution vector not null\n");
        ierr = VecCopy(ts->vec_der,th->V0);CHKERRQ(ierr);
      }else{
        PetscPrintf(PETSC_COMM_WORLD,"Tianyi Notes: Original Code, Backward euler to estimate initial derivative\n");
    ...
    // Tianyi Modifed: TSAdaptCheckStage function is used to reduce time step if solver diverged.
    //				   It's modified to allow user defined reduction factor for time step after each rejection.
    //           This function is located at scr/ts/adaapt/interface/tsadapt.c
    ...
    if (PrintFileTimeAdjust) {
      ts->ptime += ts->time_step;
  		ts->time_step = next_time_step_dummy;
  	}else{
  		if (ts->TianyiAdaptTime2) {
    		// Tianyi Modified: User defined adaptive scheme. Further adjust timestep based on lits and nits
    		ierr = SNESGetIterationNumber(ts->snes, &nits); CHKERRQ(ierr);
    		ierr = SNESGetLinearSolveIterations(ts->snes, &lits); CHKERRQ(ierr);
    		if (nits < ts->min_nits && lits < ts->min_lits) {
    			// If both non linear iteration and linear iteration smaller than desired range of its, increase timestep
    			next_time_step = ts->time_step * (1.0 + ts->TianyiAdjustFactor2);
    			PetscPrintf(PETSC_COMM_WORLD, "Tianyi Modified AdaptTime2: nits and lits smaller than desired range, increase timestep from %.3e to %.3e\n", ts->time_step, next_time_step);
    		} else if (nits > ts->max_nits || lits > ts->max_lits) {
    			// If either non linear iteration or linear iteration larger than desired range of its, decrease timestep
    			next_time_step = ts->time_step * (1.0 - ts->TianyiAdjustFactor2);
    			PetscPrintf(PETSC_COMM_WORLD, "Tianyi Modified AdaptTime2: nits or lits larger than desired range, further decrease timestep from %.3e to %.3e\n", ts->time_step, next_time_step);
    		}
      }
      ts->ptime += ts->time_step;
      ts->time_step = next_time_step;
