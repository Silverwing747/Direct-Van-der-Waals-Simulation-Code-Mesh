...
PetscErrorCode TSAdaptCheckStage(TSAdapt adapt,TS ts,PetscReal t,Vec Y,PetscBool *accept)
{
  ...
  if (!(*accept) && !ts->reason) {
    ...
    if (ts->TianyiAdaptTime1) {
      new_dt = dt * ts->TianyiAdjustFactor1; // Tianyi Modified: If TianyiAdaptTime1 is on, using user defined adjust factor for next time step after rejection
      PetscPrintf(PETSC_COMM_WORLD, "      Tianyi Modified AdaptTime1 to adjust time step. Detail see adapt monitor\n");
    } else {
      new_dt = dt * adapt->scale_solve_failed; // Tianyi Notes: Use the original method to reduce timestep after rejection
    }
