...
class VTK(object):
    ...
    def write(self, filename, nurbs,
              ...
              scalars={}, vectors={},
              sampler=None, order=0,mapped=False):
        """
        Parameters
        ----------
        filename : string
        ...
        if mapped:
            assert order<=1, "Invalid input. Only support to calculate first derivative for mapped domain!"
        else:
            assert order<=3, "Invalid input. Only support to calculate third order derivative for non-mapped domain!"
        ...
        ##  Modifed section to obtain derivatives
        Result = F
        scalars_ref = scalars.copy()
        for name,index in vectors.items():
            for i in range(len(index)):
                scalars_ref.update({name+str(i):index[i]})
        start = F.shape[-1]
        for i in range(1,order+1,1):
            ReshapeSize = np.asarray(F.shape[:-1] + (F.shape[-1]*dim**i,))
            D = nurbs.derivative(i,mapped,*uvw,**dict(fields=fields))
            Result = np.concatenate((Result,D.reshape(ReshapeSize)),axis=-1)
            for name,index in sorted(scalars_ref.items(),key=lambda x:x[1]):
                vectors.update({name+'_d'+str(i):range(start,start+dim**i)})
                start = start + dim**i
        vectors_ref = vectors.copy()
        ...
        points.shape = (-1, 3)
        fields = np.rollaxis(Result, -1).ravel('f')
        ...
        for i, (name, index) in enumerate(vectors):
            array = np.zeros((len(points),len(index)),dtype='d')
        ...
        for i, (name, array) in enumerate(vectors):
            ...
            attr_name = name or (attr_type.lower() + str(i))
            attr_name = attr_name.replace(' ', '_')
            fh_write('%s %s %s' %(attr_type, attr_name, '1'))
            fh_write('%s %d %d %s' %(attr_name, len(vectors_ref.get(name)), len(points), 'double'))
            fh_write('\n')
            array.astype('>d').tofile(fh)
            fh_write('\n')
