...
class NURBS(object):
    ...
    def gradient(self, mapped=False,u=None, v=None, w=None, fields=None):
        # Notice this function defintion is modified. IF have bug check with the original
        """
    ...
    def hessian(self, fields=None, u=None, v=None, w=None,
                mapped=False):
        """
        ...
        arglist.extend(uvw)
        #
        Hessian = getattr(_bsp, 'Hessian%d' % self.dim)
        H = Hessian(*arglist)
        #
        shape = list(H.shape)
        if squeeze: del shape[dim]
    ...
    def derivative(self, order,
                   mapped=False,u=None, v=None, w=None, fields=None):
        """
