...
int main(int argc, char *argv[]) {
  ...
  if (!iga->collocation) {
    ...
    PetscInt n = !d;
    //PetscPrintf(PETSC_COMM_WORLD,"axis: %d; d: %d; n: %d\n",user.axis,d,n);
    PetscPrintf(PETSC_COMM_WORLD,"Test.\n");
    ierr = IGAFormSetSystem(form,System,&user);CHKERRQ(ierr);
    ierr = IGAFormSetBoundaryValue(form,user.axis,d,0,10.0);CHKERRQ(ierr);
    ierr = IGAFormSetBoundaryValue(form,user.axis,n,0,0.0);CHKERRQ(ierr);
    //ierr = IGAFormSetBoundaryLoad(form,user.axis,n,0,4.0);CHKERRQ(ierr);
