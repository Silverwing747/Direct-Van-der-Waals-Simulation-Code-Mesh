...
PetscErrorCode IGASetFormSystemSemiStagger(IGA iga,IGAFormSystemSemiStagger System,void *SysCtx)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(iga,IGA_CLASSID,1);
  iga->form->ops->SystemSemiStagger = System;
  iga->form->ops->SysCtx = SysCtx;
  PetscFunctionReturn(0);
}

PetscErrorCode IGASetFormSystemStagger(IGA iga,IGAFormSystemStagger System,void *SysCtx)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(iga,IGA_CLASSID,1);
  iga->form->ops->SystemStagger = System;
  iga->form->ops->SysCtx = SysCtx;
  PetscFunctionReturn(0);
}
...
PetscErrorCode IGASetFormIFunctionStagger(IGA iga,IGAFormIFunctionStagger IFunctionStagger,void *IFunCtx)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(iga,IGA_CLASSID,1);
  iga->form->ops->IFunctionStagger = IFunctionStagger;
  iga->form->ops->IFunCtx   = IFunCtx;
  PetscFunctionReturn(0);
}
