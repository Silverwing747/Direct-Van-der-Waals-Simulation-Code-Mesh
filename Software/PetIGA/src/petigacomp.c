...
PetscErrorCode IGAComputeCoordExtrema(IGA iga,Vec vecU,
                                PetscInt n,PetscScalar S[],
                                IGAFormScalar Scalar, PetscBool max, void *ctx)
{
  // This function will return either max (max=1) or min (min=1) of gauss point coordinate
  ...
  while (IGANextElement(iga,element)) {
    ...
    while (IGAElementNextPoint(element,point)) {
      ierr = PetscMemzero(workS,(size_t)n*sizeof(PetscScalar));CHKERRQ(ierr);
      ierr = Scalar(point,U,n,workS,ctx);CHKERRQ(ierr);
      ierr = IGAPointExtremeCoord(point,n,max,workS,localS);CHKERRQ(ierr);
    }
    ierr = IGAElementEndPoint(element,&point);CHKERRQ(ierr);
  }
  ierr = IGAEndElement(iga,&element);CHKERRQ(ierr);
  ...
  /* Assemble global scalars S */
  ierr = IGAGetComm(iga,&comm);CHKERRQ(ierr);
  if (max) {ierr = MPI_Allreduce(localS,S,(PetscMPIInt)n,MPIU_SCALAR,MPIU_MAX,comm);CHKERRQ(ierr);}
  else {ierr = MPI_Allreduce(localS,S,(PetscMPIInt)n,MPIU_SCALAR,MPIU_MIN,comm);CHKERRQ(ierr);}
